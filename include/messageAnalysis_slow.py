...
_PROGRESS_TABLES = {
    "emotion": "bot_emotion",
    "intent": "bot_classification",
    "risk": "bot_riskassessment",
    "myth": "bot_mythassessment",
}
...
class chatmessage_analyze():
    ...
    def __init__(self):
        ...
        analysis_start_date = '2026-06-04'

        query = """
        SELECT DISTINCT session_id
        FROM bot_chatmessage
        WHERE sender='user'
        and timestamp >= %(analysis_start_date)s
        """
        self.sessions = pd.read_sql(query, self.source_engine, params={"analysis_start_date": analysis_start_date})["session_id"].tolist()
    ...
    def should_perform_detection(self, session_id, table_name: str) -> Tuple[bool, int, int]:
        ...
        try:
            ...
            if should_classify:
                next_threshold = thresholds[result_count]
                self.logger.debug(
                    f"[{table_name}] Session {session_id}: {total_messages} messages, "
                    f"{result_count} results so far, triggering at {next_threshold}th message"
                )
            else:
                next_threshold = thresholds[-1] if thresholds else total_messages
                self.logger.debug(
                    f"[{table_name}] Session {session_id}: {total_messages} messages, "
                    f"{result_count} results, no run needed"
                )

            return should_classify, int(total_messages), int(next_threshold)
    ...
    class emotion:
        ...
        def build_prompt(self, messages, language='en') -> str:
            ...
            for code, en, am, om in self.EMOTION_CHOICES:
                ...
                emotion_options += f"- {code}: {label} ({description})\n"
            ...
            with self.parent.destination_engine.begin() as conn:
                conn.execute(
                    text(query),
                    {
                        ...
                        "emotion_ratings": json.dumps(result["emotion_ratings"]),
                        "primary_emotion": result["primary_emotion"],
                        "confidence_score": result["confidence"],
                        "messages_analyzed": result["messages_analyzed"],
                        "analysis_context": json.dumps(result["analysis_context"])
                    }
                )

        def run_for_session(self, session_id, messages_df, language):
            """
            Analyze and save emotion results for an already-fetched slice of
            a session's messages. Used by chatmessage_analyze.process_all()
            so emotion doesn't issue its own separate DB fetch.
            """
            if messages_df is None or messages_df.empty:
                self.parent.logger.warning(f"No messages provided for emotion detection: {session_id}")
                return None

            msg_list = messages_df["message"].tolist()
            result = self.detect_user_emotions(msg_list, language)
            if not result:
                return None
        ...
        def perform_emotion_detection(self, session, force: bool = False):
            try:
                should_detect, total_messages, next_threshold = self.parent.should_perform_detection(
                    session, table_name=_PROGRESS_TABLES["emotion"]
                )
                ...
                target = total_messages if force else next_threshold
                message_limit = int(min(target, _MAX_CONTEXT_MESSAGES["emotion"]))
    ...
    class intent:
        def __init__(self, chatmessage_analyze):
            self.parent = chatmessage_analyze
            self.INTENT_CHOICES = {
                "ASK_INFO": {"en": "Ask Information", "am": "መረጃ መጠየቅ", "om": "Odeeffannoo Gaafachuu"},
                "ASK_ACTION": {"en": "Ask for Action", "am": "እርምጃ/ተግባር መጠየቅ", "om": "Tarkaanfii Gaafachuu"},
                "REPORT_INCIDENT": {"en": "Report Incident", "am": "ክስተት ማሳወቅ/ሪፖርት ማድረግ", "om": "Mudaa/Mudannoo Gabaasuu"},
                "EXPRESS_EMOTION": {"en": "Express Emotion", "am": "ስሜትን መግለጽ", "om": "Miira Agarsiisuu"},
                "ASK_CONFIDENTIALITY": {"en": "Ask About Confidentiality", "am": "ሚስጥራዊነትን መጠየቅ", "om": "Iccitiifamuu Gaafachuu"},
                "SEEK_VALIDATION": {"en": "Seek Validation", "am": "ማረጋገጫ መፈለግ", "om": "Dhugoomsa Barbaaduu"},
                "REFUSE_HELP": {"en": "Refuse Help", "am": "እርዳታ እምቢ ማለት/አለመቀበል", "om": "Gargaarsa Diduu"},
        ...
        def build_prompt(self, messages, language='en') -> str:
            ...
            for code, labels in self.INTENT_CHOICES.items():
                ...
                intent_options += f"- {code}: {label} ({description})\n"
        ...
        def classify_user_intent(self, messages, language) -> Optional[Dict]:
            ...
            try:
                ...
                try:
                    ...
                    messages_analyzed = classification_result.get('messages_analyzed', 0)
                    analysis_context = classification_result
        ...
        def run_for_session(self, session_id, messages_df, language):
            """
            Analyze and save intent classification for an already-fetched
            slice of a session's messages. Used by
            chatmessage_analyze.process_all() so intent doesn't issue its
            own separate DB fetch.
            """
            if messages_df is None or messages_df.empty:
                self.parent.logger.warning(f"No messages provided for intent classification: {session_id}")
                return None

            result = self.classify_user_intent(messages_df, language)
            if not result:
                return None

            result["session_id"] = session_id
            self.save_classification(result)
            self.parent.logger.info(f"Classification saved for session {session_id}: {result['intent']}")
            return result

        def perform_intent_classification(self, session_id: str, force: bool = False):
            try:
                should_classify, total_messages, next_threshold = self.parent.should_perform_detection(
                    session_id, table_name=_PROGRESS_TABLES["intent"]
                )
                ...
                target = total_messages if force else next_threshold
                message_limit = int(min(target, _MAX_CONTEXT_MESSAGES["intent"]))
                ...
                self.parent.logger.info(
                    f"Classification saved for session {session_id}: {classification_result['intent']}"
                )
        ...
        def process_intents(self):
            ...
            for session_id in self.parent.sessions:
                try:
                    self.perform_intent_classification(str(session_id))
                except Exception as e:
                    self.parent.logger.error(f"Failed session {session_id}: {e}")
    ...
    class risk:
        def __init__(self, chatmessage_analyze):
            self.parent = chatmessage_analyze
            self.RISK_CHOICES = {
                "ABUSE": {"en": "Abuse", "am": "ጥቃት / በደል", "om": "Miidhaa / Dararaa"},
                "DOMESTIC_VIOLENCE": {"en": "Domestic Violence", "am": "የቤት ውስጥ ጥቃት", "om": "Miidhaa Mana Keessaa"},
                "SELF_HARM": {"en": "Self Harm", "am": "ራስን መጉዳት", "om": "Of Miidhuu"},
                "ILLEGAL_ABORTION": {"en": "Unsafe / Illegal Abortion", "am": "ሕገወጥ ወይም ደህንነቱ ያልተጠበቀ ውርጃ", "om": "Ulfa Baasuu Seeraan Alaa Yookaan Nageenya Hin Qabne"},
                "SEXUAL_VIOLENCE": {"en": "Sexual Violence", "am": "ወሲባዊ ጥቃት", "om": "Miidhaa Saalaa"},
                "UNSAFE_PRACTICES": {"en": "Unsafe Practices", "am": "አደገኛ ድርጊቶች", "om": "Gochaalee Balaa Qaban"},
                "CRISIS": {"en": "Crisis", "am": "አስቸኳይ ችግር", "om": "Rakkoo Ariifachiisaa"},
                "NEUTRAL": {"en": "No Significant Risk", "am": "ከፍተኛ አደጋ አልተገኘም", "om": "Balaan Guddaan Hin Argamne"}
            }
        ...
        def build_prompt(self, messages: pd.DataFrame, language: str) -> str:
            ...
            for code, labels in self.RISK_CHOICES.items():
                ...
                risk_options += f"- {code}: {label} ({description})\n"
        ...
        def classify_user_risk(self, messages, language) -> Optional[Dict]:
            ...
            try:
                ...
                try:
                    ...
                    messages_analyzed = risk_result.get('messages_analyzed', 0)
                    analysis_context = risk_result
        ...
        def save_risk_assessment(self, result):
            ...
            """
            # NOTE: previously this was executed as a raw string instead of
            # being wrapped in text(query), which fails under SQLAlchemy Core.
            with self.parent.destination_engine.begin() as conn:
                conn.execute(
                    text(query),
                    {
        ...
        def run_for_session(self, session_id, messages_df, language):
            """
            Analyze and save risk assessment for an already-fetched slice of
            a session's messages. Used by chatmessage_analyze.process_all()
            so risk doesn't issue its own separate DB fetch.
            """
            if messages_df is None or messages_df.empty:
                self.parent.logger.warning(f"No messages provided for risk assessment: {session_id}")
                return None

            result = self.classify_user_risk(messages_df, language)
            if not result:
                return None

            result["session_id"] = session_id
            self.save_risk_assessment(result)
            self.parent.logger.info(f"Risk assessment saved for session {session_id}: {result['risk_level']}")
            return result

        def perform_riskassessment(self, session_id: str, force: bool = False):
            try:
                should_classify, total_messages, next_threshold = self.parent.should_perform_detection(
                    session_id, table_name=_PROGRESS_TABLES["risk"]
                )
                ...
                target = total_messages if force else next_threshold
                message_limit = int(min(target, _MAX_CONTEXT_MESSAGES["risk"]))
                ...
                self.parent.logger.info(
                    f"Risk assessment saved for session {session_id}: {risk_result['risk_level']}"
                )

                return risk_result

            except Exception as e:
                self.parent.logger.error(f"Error assessing risk for session {session_id}: {e}")
                return None
        ...
        def process_risk_assessments(self):
            ...
            for session_id in self.parent.sessions:
                try:
                    self.perform_riskassessment(str(session_id))
                except Exception as e:
                    self.parent.logger.error(f"Failed session {session_id}: {e}")
    ...
    class myth:
        def __init__(self, chatmessage_analyze):
            self.parent = chatmessage_analyze
            self.MYTH_TYPES = {
                "CULTURAL_HYMEN": {"en": "Hymen & Virginity Myth", "am": "የሃይመን እና ድንግልና እምነት", "om": "Amantii Hymenii fi Durbummaa"},
                "CULTURAL_MENSTRUATION": {"en": "Menstruation Myth", "am": "የወር አበባ እምነት", "om": "Amantii Marsaa Laguu"},
                "CULTURAL_FERTILITY": {"en": "Fertility Myth", "am": "የወሊድ ችሎታ እምነት", "om": "Amantii Dhalchummaa"},
                "CULTURAL_PREGNANCY": {"en": "Pregnancy Myth", "am": "የእርግዝና እምነት", "om": "Amantii Ulfaa"},
                "CULTURAL_CONTRACEPTION": {"en": "Contraception Myth", "am": "የእርግዝና መከላከያ እምነት", "om": "Amantii Karoora Maatii"},
                "MEDICAL_CONTRACEPTION": {"en": "Contraception Misconception", "am": "የእርግዝና መከላከያ የተሳሳተ ግንዛቤ", "om": "Dogoggora Karoora Maatii"},
                "MEDICAL_STI": {"en": "STI/HIV Misconception", "am": "የSTI/HIV የተሳሳተ ግንዛቤ", "om": "Dogoggora STI/HIV"},
                "MEDICAL_PREGNANCY": {"en": "Pregnancy Misconception", "am": "የእርግዝና የተሳሳተ ግንዛቤ", "om": "Dogoggora Ulfaa"},
                "MEDICAL_FERTILITY": {"en": "Fertility Misconception", "am": "የወሊድ ችሎታ የተሳሳተ ግንዛቤ", "om": "Dogoggora Dhalchummaa"},
                "MEDICAL_ANATOMY": {"en": "Anatomy Misconception", "am": "የሰውነት አካላት የተሳሳተ ግንዛቤ", "om": "Dogoggora Qaama Namaa"},
                "MEDICAL_PUBERTY": {"en": "Puberty Misconception", "am": "የጉርምስና የተሳሳተ ግንዛቤ", "om": "Dogoggora Dargaggummaa"},
                "MEDICAL_MENSTRUATION": {"en": "Menstrual Health Misconception", "am": "የወር አበባ ጤና የተሳሳተ ግንዛቤ", "om": "Dogoggora Fayyaa Marsaa Laguu"},
                "NO_MYTH": {"en": "No Myth Detected", "am": "ምንም እምነት አልተገኘም", "om": "Amantiin Hin Argamne"}
            }
        ...
        def build_prompt(self, messages: pd.DataFrame, language: str) -> str:
            ...
            for code, labels in self.MYTH_TYPES.items():
                ...
                myth_options += f"- {code}: {label} ({description})\n"
        ...
        def classify_user_myth(self, messages, language) -> Optional[Dict]:
            if messages is None or messages.empty:
            ...
            try:
                ...
                try:
                    ...
                    analysis_context = myth_result
                    correction_provided = myth_result.get('correction_provided', False)
            ...
            with self.parent.destination_engine.begin() as conn:
                conn.execute(
                    text(query),
                    {
                        ...
                        "confidence_score": result["confidence"],
                        "analysis_context": json.dumps(result["analysis_context"]),
                        "correction_provided": result["correction_provided"]
                    }
                )

        def run_for_session(self, session_id, messages_df, language):
            """
            Analyze and save myth assessment for an already-fetched slice of
            a session's messages. Used by chatmessage_analyze.process_all()
            so myth doesn't issue its own separate DB fetch.
            """
            if messages_df is None or messages_df.empty:
                self.parent.logger.warning(f"No messages provided for myth assessment: {session_id}")
                return None

            result = self.classify_user_myth(messages_df, language)
            if not result:
                return None

            result["session_id"] = session_id
            self.save_myth_assessment(result)
            self.parent.logger.info(f"Myth assessment saved for session {session_id}: {result['myth_type']}")
            return result

        def perform_mythassessment(self, session_id: str, force: bool = False):
            try:
                should_classify, total_messages, next_threshold = self.parent.should_perform_detection(
                    session_id, table_name=_PROGRESS_TABLES["myth"]
                )
                ...
                target = total_messages if force else next_threshold
                message_limit = int(min(target, _MAX_CONTEXT_MESSAGES["myth"]))
                ...
                myth_result = self.classify_user_myth(messages_df, language)
                ...
                self.parent.logger.info(
                    f"Myth assessment saved for session {session_id}: {myth_result['myth_type']}"
                )

                return myth_result

            except Exception as e:
                self.parent.logger.error(f"Error assessing myth for session {session_id}: {e}")
                return None
        ...
        def process_myth_assessments(self):
            ...
            for session_id in self.parent.sessions:
                try:
                    self.perform_mythassessment(str(session_id))
                except Exception as e:
                    self.parent.logger.error(f"Failed session {session_id}: {e}")
